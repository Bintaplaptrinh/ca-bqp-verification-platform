...
```bash
pip install -e '.[dev]'
pip install --no-deps -e vendor/vietocr        # OCR fallback recognizer; see vendor/README.md
python -m pytest -q
...
environment has the whole AI stack installed (easyocr/vietocr/paddleocr/torch/
...
OCR defaults to EasyOCR's Vietnamese Latin recognizer with exactly one configured fallback: **VietOCR** (`easyocr` detector + `vietocr` recognizer). VietOCR is recognition-only — it transcribes one cropped line and has no detector — so the fallback engine reuses EasyOCR's CRAFT detection stage (`EasyOcrDetector`) and swaps only the recognizer. The fallback runs when the primary's quality gate fails, when the source image is blurred/low-contrast, **or when the primary's mean recognition confidence falls below `OCR_FALLBACK_CONFIDENCE_MIN` (0.70)** — a short key/value scan produces too few lines to fail the distribution metrics even when the recognizer itself is unsure, so the gate alone missed exactly the readings worth re-running. `run_ocr`'s evidence records `primary_confidence`, `fallback_reason` and `fallback_confidence`. It still selects one engine's complete output and never splices or votes on characters. When the two engines disagree on identity fields the gate fails (`parse_quality.critical_disagreement=true`) and confidence drops to zero — this is a deliberate fail-closed path, not a bug: a garbled scan that both engines mis-transcribe differently must abstain, not average the two guesses. EasyOCR/VietOCR parameters are settings, not document-specific replacements, and Docker bakes all model assets so runtime remains offline.

The Paddle and Tesseract OCR engines were removed (`PaddleEngine`, `PaddleDetector`, `PaddleRecognizer`, `TesseractEngine`, `preprocess_for_tesseract`, the `pytesseract` dependency and the `tesseract-ocr` apt packages). `paddleocr` remains a dependency for **PP-StructureV3 table-structure recognition only** (`document_intelligence/table.py`), which is a different path from text recognition; do not reintroduce it as an OCR engine.

VietOCR is vendored at `apps/backend/vendor/vietocr` (inside the Docker build context) (upstream pbcquoc/vietocr, MIT) and installed with `pip install --no-deps -e apps/backend/vendor/vietocr`, never from PyPI: the published sdist hard-pins `pillow==10.2.0`, which pdfplumber's `Pillow>=12.2` rules out. Its runtime needs (`einops`, `gdown>=5.2`, torch/torchvision, PyYAML) are declared in `apps/backend/pyproject.toml` instead — `gdown<5` is unusable because `vietocr.tool.utils` imports it at module import time and it imports `pkg_resources`, which setuptools>=81 no longer ships. `engines.py` also refuses upstream's `Cfg.load_config_from_name()`: that fetches both YAML configs from vocr.vn at construction time, so the engine reads `apps/backend/vendor/vietocr/config/base.yml` + the architecture file from disk (`VIETOCR_CONFIG_DIR` overrides the location; the image sets `/opt/vietocr/config`). The checkpoint is `VIETOCR_WEIGHTS` when it exists (baked into the image, with `VIETOCR_DOWNLOAD_ENABLED=false`), otherwise downloaded once — `.partial` then renamed, so an interrupted download is never picked up as a valid checkpoint — into `~/.cache/cabqp/vietocr/`.
...
Tests that need a real engine or server (OCR inference, the audit triggers) are skipped or stubbed rather than silently passing. The Docker image bakes the EasyOCR, VietOCR, PP-Structure and embedding models at build time, so a container never fetches models at runtime. On Windows, EasyOCR models live in the configured/user cache and `USE_TF=0 USE_TORCH=1` must be set in the process environment (see Document intelligence above). `tests/test_ocr_pipeline_and_structure.py` fakes both engines: note that `get_settings` is `lru_cache`d, so an env override needs the file's `settings_env` fixture (which clears the cache) — `monkeypatch.setenv` alone changes nothing.
