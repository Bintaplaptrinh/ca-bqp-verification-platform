...
No Docker, Makefile, or CI exists yet — don't assume `docker compose up`, `make ...`, or a GitHub Actions workflow will work until those are actually added.
...
The repository path contains non-ASCII characters (`Chuyên-đề`). `pip install -e .` (editable install) fails on Windows with a `UnicodeEncodeError` from setuptools' `.pth` file writer under this path. Install dependencies directly instead (`pip install fastapi uvicorn pytest httpx`) rather than an editable install of the local `cabqp` package; `pythonpath = ["src"]` in `pyproject.toml` makes the package importable for tests without it.
